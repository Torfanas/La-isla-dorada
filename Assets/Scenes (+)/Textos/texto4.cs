...
{
    public GameObject messagePanel3; // Referencia al panel de la UI
    private bool hasBeenActivated = false; // Variable para verificar si ya se activó

    void Start()
    {
        if (messagePanel3 != null)
            messagePanel3.SetActive(false); // Asegurarse de que el panel comience oculto
    }
    ...
    void ShowPanel()
    {
        if (messagePanel3 != null)
        {
            messagePanel3.SetActive(true); // Muestra el panel
    ...
    {
        if (messagePanel3 != null)
            messagePanel3.SetActive(false); // Oculta el panel
    }
}
